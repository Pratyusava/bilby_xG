...
"""Waveform source models for next-generation-detector inference.

Vendored and lightly adapted from the ``freq_dep_antenna_response_HM`` branch of
the bilby fork. In addition to the standard CBC source models it provides the
*individual-mode* models required by the ``*NextGeneration`` likelihoods, which
return a per-azimuthal-mode dictionary of plus/cross polarisations so the
frequency-dependent antenna response can be applied mode-by-mode:

* :func:`lal_binary_black_hole_individual_modes`
* :func:`lal_binary_black_hole_relative_binning_individual_modes`
...
def lal_binary_black_hole_individual_modes(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, **kwargs):
    """ A Binary Black Hole waveform model using lalsimulation returning the
    individual modes of the waveform if multiple modes are requested
    ...
    for m_ in [2, 3, 4]:
        waveform_kwargs['mode_array'] = [
            mode for mode in mode_array if np.abs(mode[-1]) == m_]
        if waveform_kwargs['mode_array']:
            return_dict[str(m_)] = _base_lal_cbc_fd_waveform(
                frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
                luminosity_distance=luminosity_distance, theta_jn=theta_jn, phase=phase,
                a_1=a_1, a_2=a_2, tilt_1=tilt_1, tilt_2=tilt_2, phi_12=phi_12,
                phi_jl=phi_jl, **waveform_kwargs)
...
def lal_binary_black_hole_relative_binning_individual_modes(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, fiducial, **kwargs):
    """ Source model to go with RelativeBinningGravitationalWaveTransient likelihood.
    ...
    if fiducial == 1:
        for ell, emm in mode_array:
            waveform_kwargs['mode_array'] = [[ell, emm]]
            return_dict[f"{ell},{emm}"] = _base_lal_cbc_fd_waveform(
                frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
                luminosity_distance=luminosity_distance, theta_jn=theta_jn, phase=phase,
                a_1=a_1, a_2=a_2, tilt_1=tilt_1, tilt_2=tilt_2, phi_12=phi_12,
                phi_jl=phi_jl, **waveform_kwargs)
        ...
        for ell, emm in mode_array:
            waveform_kwargs['mode_array'] = [[ell, emm]]
            return_dict[f"{ell},{emm}"] = _base_waveform_frequency_sequence(
                frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
                luminosity_distance=luminosity_distance, theta_jn=theta_jn, phase=phase,
                a_1=a_1, a_2=a_2, tilt_1=tilt_1, tilt_2=tilt_2, phi_12=phi_12, lambda_1=0.0, lambda_2=0.0,
                phi_jl=phi_jl, **waveform_kwargs)
    waveform_kwargs['mode_array'] = mode_array
    return return_dict
...
def binary_black_hole_individual_modes_frequency_sequence(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, **kwargs):
    """ A Binary Black Hole waveform model using lalsimulation. This generates
    a waveform only on specified frequency points with strain values for individual modes. This is useful for
    likelihood requiring waveform values at a subset of all the frequency
    samples. For example, this is used for MBGravitationalWaveTransientNextGeneration.
    ...
    """
    waveform_kwargs = dict(
        waveform_approximant='IMRPhenomHM', reference_frequency=50.0,
        catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
        pn_phase_order=-1, pn_amplitude_order=0)
    waveform_kwargs.update(kwargs)
...
def _base_waveform_individual_modes_frequency_sequence(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, lambda_1, lambda_2, phi_jl, theta_jn, phase,
        **waveform_kwargs):
    """ Generate a cbc waveform model on specified frequency samples with strains corresonding to individual modes
    ...
    for m_ in [2, 3, 4]:
        ...
        try:
            h_plus, h_cross = lalsim_SimInspiralChooseFDWaveformSequence(
                ...
                spin_2z, reference_frequency, luminosity_distance, iota,
                waveform_dictionary, approximant, frequencies)
            return_dict[str(m_)] = dict(plus=h_plus.data.data, cross=h_cross.data.data)
        except Exception as e:
            if not catch_waveform_errors:
                raise
            else:
